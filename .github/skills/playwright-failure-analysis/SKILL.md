...
**Objective:** Automatically locate, parse, and diagnose Playwright test failures by reading the JUnit XML report using your standard file access tools, and efficiently report the results based on the volume of failures using standard templates.
...
3. **Analyze and Synthesize (Scale Rule):** 
   ...
   - **If there are MORE than 5 failures:** 
     ...
     1. Use your file reading tool to read the format from `file-template.md` in this skill's folder.
     2. Use your file creation/writing tool to create a well-formatted markdown file named `playwright-failure-summary.md` in the root directory using the `file-template.md` structure.
     3. In this file, group the errors by root cause (e.g., "Timeout Errors", "Assertion Failures on Login", "Network Issues").
     4. For each group in the file, list the affected tests/files and propose the overarching fix.
     5. In the chat window, ONLY output a brief executive summary (e.g., *"Found 50 failures across 3 main categories. I have generated a detailed report for you at `playwright-failure-summary.md`."*).
